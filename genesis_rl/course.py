...
def path_segments(spec: CourseSpec, rail_half: float = 0.6, rail_w: float = 0.06,
                  thickness: float = 0.05, drop: float = 0.4
                  ) -> list[tuple[tuple[np.ndarray, np.ndarray], tuple[np.ndarray, np.ndarray]]]:
    """本番シムの青パス=「ゲート内側を通る帯(細い縁レール2本＋半透明フィル)」を区間毎に返す。

    実映像の青パスは床ではなく、ゲート開口の内側(中心より下・内側下端との間)を貫いて次の
    ゲートへ続く3Dパス。飛行中心線 spec.ribbon_pts(=ゲート中心を通る)を鉛直下方へ drop だけ
    下げ(内側の下寄りへ)、その線に沿って:
      - fill : rail 間を埋める幅 2*rail_half の薄い帯(scene側で半透明・淡発光)
      - rails: ±rail_half の細い2本を1メッシュに結合(scene側で明るい発光)
    を生成して (fill, rails) で返す。区間数・並びは ribbon_segments と一致するので、
    genesis_race_env の ribbon_entities 表示/明滅機構をそのまま流用できる(scene側でfillを
    ribbon_entities、railsを ribbon_rail_entities に積み、_update_ribbon が両者を同期移動)。
    """
    per = RIBBON_SAMPLES_PER_SEG
    ...
    for i in range(1, spec.n_gates):
        ...
        # fill: 縁レール間を埋める薄い帯
        fill = ribbon_mesh(pts, width=2.0 * rail_half, thickness=thickness * 0.5)
        # rails: 左右2本を1メッシュに結合
        vL, fL = ribbon_mesh(pts + side * rail_half, width=rail_w, thickness=thickness)
        vR, fR = ribbon_mesh(pts - side * rail_half, width=rail_w, thickness=thickness)
        rails = (np.vstack([vL, vR]),
                 np.vstack([np.asarray(fL), np.asarray(fR) + len(vL)]))
        segs.append((fill, rails))
    return segs
