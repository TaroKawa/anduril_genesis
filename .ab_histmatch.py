...
# --- 参照(Madrona)ヒストグラムへのper-channelマッチングLUT ---
import cv2, glob
_ref_files = sorted(glob.glob("runs/genesis_ab_madrona/frame_*.png"))
_ref = np.stack([cv2.cvtColor(cv2.imread(f), cv2.COLOR_BGR2RGB) for f in _ref_files[:100]])
_ref_cdf = []
for c in range(3):
    h, _ = np.histogram(_ref[..., c], bins=256, range=(0, 256))
    _ref_cdf.append(np.cumsum(h) / h.sum())

def hist_match_lut(img_u8: np.ndarray) -> np.ndarray:
    out = img_u8.copy()
    for c in range(3):
        h, _ = np.histogram(img_u8[..., c], bins=256, range=(0, 256))
        cdf = np.cumsum(h) / max(h.sum(), 1)
        lut = np.searchsorted(_ref_cdf[c], cdf).clip(0, 255).astype(np.uint8)
        out[..., c] = lut[img_u8[..., c]]
    return out
...
def act(rgb_u8, vec, done_prev):
    img = rgb_u8[0].cpu().numpy()
    img = hist_match_lut(img)
    rgb_u8 = torch.as_tensor(img).unsqueeze(0).to(rgb_u8.device)
